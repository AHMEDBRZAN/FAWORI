...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 5), (_) {
      if (_ctrl.hasClients) _ctrl.nextPage(duration: const Duration(milliseconds: 400), curve: Curves.easeInOut);
    });
  }
  ...
  Widget _banner(int i) {
    if (_homeImages.isNotEmpty) {
      return Image.network(ImagesService.remoteUrl(_homeImages[i % _homeImages.length]),
          fit: BoxFit.cover, width: double.infinity, errorBuilder: (_, __, ___) => const _Fallback());
    }
  ...
  Widget _quick(IconData ic, String label, Color c, VoidCallback? onTap) => Pressable(
    ...
    child: Container(padding: const EdgeInsets.all(14),
      ...
      child: Row(children: [Icon(ic, color: c, size: 26), const SizedBox(width: 10),
      ...
      body: SafeArea(bottom: false,
        child: ListView(padding: const EdgeInsets.all(16), children: [
          Row(children: [
            ...
            Expanded(child: Text(s.tr('appName'), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900))),
            IconButton(icon: const Icon(Icons.notifications_none_rounded), onPressed: () {}),
          ]),
          const SizedBox(height: 14),
          SizedBox(height: 170,
            child: PageView.builder(controller: _ctrl, itemCount: _count,
                onPageChanged: (i) => setState(() => _idx = i),
                itemBuilder: (_, i) => ClipRRect(borderRadius: BorderRadius.circular(18), child: _banner(i)))),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center,
            children: List.generate(_count, (i) => AnimatedContainer(
                ...
                decoration: BoxDecoration(color: i == _idx ? AppColors.orange : Colors.grey.shade400,
                    borderRadius: BorderRadius.circular(4))))),
          const SizedBox(height: 18),
          Text(s.isArabic ? 'الوصول السريع' : 'Quick access', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: _quick(Icons.inventory_2_rounded, s.isArabic ? 'المنتجات' : 'Products', AppColors.orange, widget.onOpenProducts)),
            const SizedBox(width: 10),
            Expanded(child: _quick(Icons.redeem_rounded, s.isArabic ? 'الهدايا' : 'Gifts', AppColors.teal, widget.onOpenProducts)),
          ]),
          const SizedBox(height: 18),
          Text(s.isArabic ? 'العلامات' : 'Brands', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
          const SizedBox(height: 10),
...
class _Fallback extends StatelessWidget {
  const _Fallback();
  @override
  Widget build(BuildContext context) => Container(color: AppColors.orange.withAlpha(40),

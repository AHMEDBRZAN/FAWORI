...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /// دوران تلقائي يتوقف نهائياً عند آخر صورة
  void _startAuto() {
    _timer?.cancel();
  ...
  Widget _gradText(String t, double size) => ShaderMask(
      ...
      child: Container(
          ...
          decoration: BoxDecoration(
              gradient: LinearGradient(colors: [
                c.withAlpha(30), Theme.of(context).colorScheme.surface
  ...
  Widget _brand(String ar, String en, Color c) => Pressable(
      ...
      child: Container(
          decoration: BoxDecoration(
              gradient: LinearGradient(colors: [
                c.withAlpha(26), Theme.of(context).colorScheme.surface
              ], begin: Alignment.topCenter, end: Alignment.bottomCenter),
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: c.withAlpha(60))),
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(en, style: TextStyle(color: c, fontSize: 20, fontWeight: FontWeight.w900)),
            const SizedBox(height: 4),
            Text(ar, style: TextStyle(color: Colors.grey.shade400, fontSize: 12)),
          ])));
                          ...
                          decoration: BoxDecoration(
                              gradient: i == _idx
                                  ? const LinearGradient(
                                      colors: [AppColors.orange, AppColors.teal])
                                  : null,
                              color: i == _idx ? null : Colors.grey.shade600,
                              borderRadius: BorderRadius.circular(4))))),
